...
# 🌿 RESTAURANTE JUGERIA "KLETA" — Backend Spring Boot
...
producto.setNombre_producto("Jugo_de_mango");
prodcuto.settipo_producto("refesco");
```
...
Le dice a Java cómo es un Producto. Tiene los mismos campos que la tabla en MySQL:
`ID_Producto`, `nombre_produto_`, `tipo_producto`, `precio`.
...
"dame todos los productos" y él va a MySQL y los trae.
...
Aquí es donde va la **lógica de negocio**, por ejemplo: verificar que el nombre_producto no esté repetido antes de guardar.
...
Por ejemplo, cuando el JS hace `fetch("/api/productos")`, el Controller es el primero en responder.
